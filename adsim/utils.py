...
def calc_tes(data, user_visit_no, ranks_list):
    ...
    """
    start_time = time.perf_counter()
    mask = data['user_visit_no'] == user_visit_no
    for rank in ranks_list:
        X = construct_X(data, user_visit_no=user_visit_no, ad_rank=rank)
        cf = config.forests[rank]
        data.loc[mask, f"te_{rank}"] = cf.const_marginal_effect(X[mask])
    finish_time = time.perf_counter()
...
def calc_base_ad_ctr_vector(data, user_visit_no):
    start_time = time.perf_counter()
    # Use cf_1 (the rank-1 forest) to estimate y0.
    X = construct_X(data, user_visit_no=user_visit_no, ad_rank=1)
    mask = data['user_visit_no'] == user_visit_no
    cf_1 = config.forests[1]
    tau_1 = cf_1.const_marginal_effect(X[mask]).reshape(-1, 1)
    m_1 = config.helpers["m1"].predict(X[mask]).reshape(-1, 1)
    e_1 = config.helpers["e1"].predict_proba(X[mask]).reshape(-1, 1)
    y_0 = m_1 - tau_1 * e_1
    finish_time = time.perf_counter()
...
def calc_split_tes(data, split_no, user_visit_no, ranks_list):
    ...
    """
    start_time = time.perf_counter()
    mask = (data['user_visit_no'] == user_visit_no) & (data['split'] == split_no)
    if not mask.any():
        finish_time = time.perf_counter()
        print(f"no rows for split {split_no} visit {user_visit_no}; skipped in {finish_time - start_time} seconds")
        return
    split_bucket = config.split_forests[split_no]
    for rank in ranks_list:
        ...
        cf_split = split_bucket[rank]
        cf_actual = config.forests[rank]
        data.loc[mask, f"te_{rank}_s"] = cf_split.const_marginal_effect(X_s.loc[mask])
        data.loc[mask, f"te_{rank}"] = cf_actual.const_marginal_effect(X.loc[mask])
    finish_time = time.perf_counter()
    print(f"finished calculating te's for rank {rank} in {finish_time - start_time} seconds")
...
def calc_base_ad_split_ctr(data, split_no, user_visit_no):
    ...
    # Use cf_1 (the rank-1 forest) to estimate y0.
    X = construct_X(
        data[(data['user_visit_no'] == user_visit_no) & (data['split'] == split_no)],
        user_visit_no=user_visit_no, ad_rank=1,
    )
    cf_1 = config.forests[1]
    X_filt = X[data['user_visit_no'] == user_visit_no]
    tau_1 = cf_1.const_marginal_effect(X_filt).reshape(-1, 1)
    m_1 = config.helpers["m1"].predict(X_filt).reshape(-1, 1)
    e_1 = config.helpers["e1"].predict_proba(X_filt).reshape(-1, 1)
    y_0 = m_1 - tau_1 * e_1
