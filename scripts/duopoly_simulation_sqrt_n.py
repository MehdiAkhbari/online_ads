...
# read data
data = pd.read_stata("..\\data\\Simulation Data - Last 2 Days - Split 3 and 4 - Aggregate.dta")

# update the split numbers so that they are 1, 2
data.loc[data['split'] == 3,'split'] = 1
data.loc[data['split'] == 4,'split'] = 2
data['split'] = 2 - (data['split'] % 2)
...
def simulate_and_save_chunk(chunk_data, chunk_id):
    ...
    filename = f"..\\results\\Duopoly Simluation Results - sqrt n - chunk {chunk_id+1} - split 7,8.dta"
